...
export const EditorTinyMCE = (props: {
    ...
    const editorSetup = useCallback((editor: Editor) => {
        ...
        editor.addCommand('contentMargin', () => {
            const oldValue = editor.dom.getStyle(editor.getBody(), 'padding');
            const newValue = prompt('Margin value', oldValue || '20px');
            editor.dom.setStyle(editor.getBody(), 'padding', newValue);
            handlerContantChange(editor.getContent(), editor);
        });

        editor.ui.registry.addMenuItem('contentMargin', {
            text: 'Page margin',
            onAction: () => editor.execCommand('contentMargin'),
            context: 'any'
        });
    }, []);
    ...
    const initProps = {
        ...
        menu: {
            ...defaultMenu, 
            view: { ...defaultMenu.view, items: defaultMenu.view.items + ' | contentMargin' }
        },
